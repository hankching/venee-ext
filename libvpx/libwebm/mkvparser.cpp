...
#ifdef MTK_AOSP_ENHANCEMENT  //added by vend_am00033 start for seeking backward
...
int Track::Info::Copy(Info& dst) const {
  ...
  if (int status = CopyStr(&Info::codecNameAsUTF8, dst))
    return status;
#ifdef MTK_AOSP_ENHANCEMENT   //Need Refine: need delete to use android language
#if defined(MTK_AUDIO_CHANGE_SUPPORT) || defined(MTK_SUBTITLE_SUPPORT)
	if (int status = CopyStr(&Info::languageAsUTF8, dst))		return status;
#endif
#endif
...
#ifdef MTK_AOSP_ENHANCEMENT   
#if defined(MTK_AUDIO_CHANGE_SUPPORT) || defined(MTK_SUBTITLE_SUPPORT)
const char* Track::GetLanguageAsUTF8() const { return m_info.languageAsUTF8;}
#endif
#endif
...
long SubTtTrack::Parse(
    ...
    SubTtTrack*& pTrack){
    if (pTrack)        return -1;
    if (i.type != Track::kSubtitle)        return -1;
    IMkvReader* const pReader = pSegment->m_pReader;
  ...
  while (pos < track_stop) {
    ...
    if (id == 0x60) {  // VideoSettings ID
...
#ifdef MTK_AOSP_ENHANCEMENT
			if (size < 0){	ALOGE("Tracks::ParseTrackEntry, AudioSettings (id == 0x61) size < 0 !");
				return E_FILE_FORMAT_INVALID;
			}
#endif
      a.start = start;
...
#ifdef MTK_AOSP_ENHANCEMENT
		if (size < 0) {	ALOGE("Tracks::ParseTrackEntry, ContentEncodings (id == 0x2D80) size < 0!");
				return E_FILE_FORMAT_INVALID;
			}
#endif
      e.start = start;
...
#ifdef MTK_AOSP_ENHANCEMENT
		 if (size <= 0)	{ ALOGE("Tracks::ParseTrackEntry, Codec Private (id == 0x23A2) size =%lld <= 0, return error!",size);
				return E_FILE_FORMAT_INVALID;
			}
#endif
      const size_t buflen = static_cast<size_t>(size);
      ...
      info.seekPreRoll = UnserializeUInt(pReader, pos, size);
			...
			const long status = UnserializeString(pReader, pos, size, info.languageAsUTF8);
			if (status)	return status;
		}
#endif
#endif
...
#ifdef MTK_AOSP_ENHANCEMENT    
long long Block::GetDataSize() const /*data number in block*/{	return m_datasize;}
#endif
