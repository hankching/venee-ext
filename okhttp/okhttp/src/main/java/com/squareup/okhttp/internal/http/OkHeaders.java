...
public final class OkHeaders {
  ...
  public static List<Challenge> parseChallenges(Headers responseHeaders, String challengeHeader) {
    ...
    for (int i = 0, size = responseHeaders.size(); i < size; i++) {
      ...
      String value = responseHeaders.value(i);//www-auth:xxxxxxx
      String originValue = value;

      //check how many scheme
      int schemeCount = 1;
      String splitter = " realm=\"";
      String splitterReg = "[r|R][e|E][a|A][l|L][m|M]=\"";
      String[] schemeParts = value.split(splitterReg);
      schemeCount = schemeParts.length - 1;
      ...
      for (int l = 0; l < schemeCount; l++) {
          //re-construct each scheme
          if (schemeCount == 1) {
              value = schemeParts[l] + splitter +
                      schemeParts[l + 1].trim();
          } else {
              if (l == 0) {
                  value = schemeParts[l] + splitter +
                          schemeParts[l + 1].substring(0,
                          schemeParts[l + 1].lastIndexOf(",")).trim();
              } else if (l == schemeCount - 1) {
                  value = schemeParts[l].substring(schemeParts[l].lastIndexOf(",") + 1).trim() +
                          splitter +
                          schemeParts[l + 1];
              } else {
                  value = schemeParts[l].substring(schemeParts[l].lastIndexOf(",") + 1).trim() +
          ...
          while (pos < value.length()) {
            ...
            pos++; // Consume ',' comma.
            pos = HeaderParser.skipWhitespace(value, pos);
            result.add(new Challenge(scheme, realm));  // add after parsing realm
            */
            ...
            String[] fields = rest.split(","); //realm=a, realm=b, realm=c
            int j = 0;
            for (String field : fields) {
                System.out.println("field[" + j + "]: " + field); //realm=a
