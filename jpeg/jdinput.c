...
latch_quant_tables (j_decompress_ptr cinfo)
{
  ...
  for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
    compptr = cinfo->cur_comp_info[ci];
#endif
  // google issue for progressive jpeg with comps_in_scan number smaller than num_components
  // quantization table for chroma component will be missed for latch, 
  // modify for latch all quantization table according to num_components
  for (ci = 0; ci < cinfo->num_components /* comps_in_scan*/ ; ci++) {
    compptr = &(cinfo->comp_info[ci]) /* cinfo->cur_comp_info[ci] */; 
