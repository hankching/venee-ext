...
DGifGetScreenDesc(GifFileType *GifFile)
{
    ...
    LOGD("Current Buf[0] = %u, Buf[1] = %u, Buf[2] = %u\n", Buf[0],Buf[1],Buf[2]);
	...
	LOGD("GifFile->SColorResolution = %u, SortFlag = %u, BitsPerPixel = %u, GifFile->SBackGroundColor = %u", \
		GifFile->SColorResolution, SortFlag, BitsPerPixel, GifFile->SBackGroundColor);
	
    if (Buf[0] & 0x80) {    /* Do we have global color map? */
	int i;


		LOGD("We have global color map\n");
        GifFile->SColorMap = GifMakeMapObject(1 << BitsPerPixel, NULL);
		LOGD("GifFile->SColorMap->ColorCount = %u\n",GifFile->SColorMap->ColorCount);
	...
	GifFile->SColorMap->SortFlag = SortFlag;
		...
		LOGD("We copied %d colors in global color map\n",i);
    } else {
        GifFile->SColorMap = NULL;
    }
