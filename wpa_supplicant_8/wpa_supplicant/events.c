...
#ifdef CONFIG_WAPI_SUPPORT
		if (!wapi && (ssid->key_mgmt & (WPA_KEY_MGMT_WAPI_CERT
				       ...
				       union wpa_event_data *data)
{
	...
	if (wpa_s->key_mgmt == WPA_KEY_MGMT_WAPI_CERT ||
		...
		*/
		...
		wpa_printf(MSG_DEBUG,"[Debug-WAPI] AP MAC address "MACSTR
		...
		wpa_printf(MSG_DEBUG,"[Debug-WAPI ^_^!!] "
			"Ready send associate complete evert to WAPI Modules!!!");
		wapi_set_msg(CONN_ASSOC, &bssid_s, &own_s, wpa_s->bss_wapi_ie ,
			wpa_s->bss_wapi_ie_len);
		return;
	}
#endif
...
*    a. Set listen channel from GO negotiation Request. This patch can fix wrong channel
*	for unkown peer invitation.
*    b. Reject auto-connection if GROUP-REMOVE command comes from user's request
*    c. Set operating channel from GO negotiation Confirmation to prevent from wrong
*	channel operation
