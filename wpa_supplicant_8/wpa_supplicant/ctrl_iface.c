	...
	const struct wpa_bss *bss, char *buf, size_t buflen)
{
	...
	pos = wpa_supplicant_wps_ie_txt(wpa_s, pos, end, bss);
		...
		if (!ie && !ie2 && !osen_ie && !iewapi && (bss->caps & IEEE80211_CAP_PRIVACY)) {
	...
	char *name, char *value)
{
	...
	char *old_value = NULL;
	char *new_value = NULL;
	old_value = wpa_config_get(ssid, name);
	if (!old_value && os_strcmp("NULL", value) == 0) {
		return 0;
	}
#endif
...
#if CONFIG_MEDIATEK
	new_value = wpa_config_get(ssid, name);
	if (!old_value && !new_value) {
		return 0;
	} else if (old_value && new_value && os_strcmp(old_value, new_value) == 0) {
		goto free_memory;
	}
#endif
	wpa_printf(MSG_INFO, "update network");
				     ...
				     char *buf, size_t buflen)
{
	...
	if (os_strncasecmp(cmd, "BEAMPLUS_", os_strlen("BEAMPLUS_")) == 0) {
		...
		if (!p2p) {
			wpa_printf(MSG_DEBUG, "Driver Beamplus: "
				"can't continue because p2p is null");
			return -1;
		}

		if (os_strncasecmp(cmd, "BEAMPLUS_GO_RESERVE_START",
			os_strlen("BEAMPLUS_GO_RESERVE_START")) == 0) {
		...
		} else if (os_strncasecmp(cmd, "BEAMPLUS_GO_RESERVE_END",
			os_strlen("BEAMPLUS_GO_RESERVE_END")) == 0) {
			wpa_printf(MSG_DEBUG, "BEAMPLUS GO Reserve End");
			p2p->in_beam_go_reserve = 0;
		} else {
			if (os_strncasecmp(cmd, "BEAMPLUS_START",
				os_strlen("BEAMPLUS_START"))==0)
				start = 1;
			else if (os_strncasecmp(cmd, "BEAMPLUS_STOP",
				os_strlen("BEAMPLUS_STOP"))==0) {
				start = 0;
				p2p->in_beam_go_reserve = 0;
			}
			wpa_printf(MSG_DEBUG, "Driver Beamplus %s",
				start == 1 ? "START" : "STOP");
...
static void wpa_supplicant_req_full_band(void *eloop_ctx,
					void *timeout_ctx);

void wpa_supplicant_req_5g_scan(void *eloop_ctx,
				void *timeout_ctx);

void stage_scan_2G_handler(struct wpa_supplicant *wpa_s,
		struct wpa_scan_results *scan_res) {

	if (scan_handler)
		scan_handler(wpa_s, scan_res);
	wpa_printf(MSG_DEBUG, "[STAGE_SCAN] Schedule 5G scan request(with scan_handler callback): 0 sec");
...
void stage_scan_5G_handler(struct wpa_supplicant *wpa_s,
		struct wpa_scan_results *scan_res) {
					...
					void *timeout_ctx)
{
	...
	reply_len = wpa_supplicant_driver_cmd(wpa_s, "SET_BAND_DUAL",
						reply, reply_size);
	wpa_s->setband = WPA_SETBAND_AUTO;
				...
				void *timeout_ctx)
{
	...
	wpa_s->scan_res_handler = stage_scan_5G_handler;
	wpa_supplicant_req_scan(wpa_s, 0, 0);
}
	...
	struct wpa_supplicant *wpa_s, char *reply, size_t reply_size)
{
	...
	wpa_s->scan_res_handler = stage_scan_2G_handler;
	wpa_supplicant_req_scan(wpa_s, 0, 0);
...
#ifdef CONFIG_MEDIATEK_WIFI_BEAM
	if (p2p && p2p->in_beam_go_reserve) {
		wpa_printf(MSG_DEBUG,
			   "SCAN: in_beam_go_reserve, skip scan");
...
#ifdef CONFIG_MTK_P2P
	if (wpa_s->p2p_group) {
		wpa_printf(MSG_INFO, "Update GO vendor elements");
		p2p_group_force_beacon_update_ies(wpa_s->p2p_group);
	}
#endif
#endif /* CONFIG_P2P */
}
