...
static void wpas_send_action_cb(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (timeout_ctx) {
		res = wpa_drv_send_action(iface, wpa_s->pending_action_freq, *(unsigned int *)timeout_ctx,
...
#ifdef CONFIG_MTK_P2P
		/**
		 * Here wpas_send_action_done is necessary: clear previous p2p
		 * action and trigger a new radio work
		 * to avoid waiting for timeout. the most often scenario is:
		 * when doing a p2p action tx, the fwk
		 * send a p2p_find
		 */
		wpa_printf(MSG_DEBUG, "Off-channelx: do a more check to avoid still "
				"some scan can't run");
		wpas_send_action_done(wpa_s);
#endif
		return;
	}
...
#ifdef CONFIG_MTK_P2P
		int *wait = os_zalloc(sizeof(unsigned int));
		if (wait)
			*wait = wait_time;
		eloop_register_timeout(0, 0, wpas_send_action_cb, wpa_s, wait);
#else
