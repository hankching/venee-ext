...
#ifdef CONFIG_MTK_P2P
		if (ssid) {
			wpa_printf(MSG_DEBUG, "P2P: f_go ssid->go_p2p_dev_addr: "
				MACSTR, MAC2STR(ssid->go_p2p_dev_addr));
			wpa_printf(MSG_DEBUG, "P2P: f_go ssid->bssid: " MACSTR,
				MAC2STR(ssid->bssid));
			if (!is_zero_ether_addr(ssid->go_p2p_dev_addr))
				os_memcpy(p2p->f_go.bssid, ssid->go_p2p_dev_addr, ETH_ALEN);
			if (is_zero_ether_addr(p2p->f_go.bssid) && !is_zero_ether_addr(ssid->bssid))
				os_memcpy(p2p->f_go.bssid, ssid->go_p2p_dev_addr, ETH_ALEN);
		}
#endif
		wpa_s->reassociate = 0;
...
#ifdef CONFIG_MTK_P2P
		wpa_printf(MSG_DEBUG, "P2P: f_go wpa_s->bssid" MACSTR, MAC2STR(wpa_s->bssid));
		if (is_zero_ether_addr(p2p->f_go.bssid))
			os_memcpy(p2p->f_go.bssid, wpa_s->bssid, ETH_ALEN);
#endif
#ifdef CONFIG_MTK_SCC
		/* code for p2p wifi vs legacy wifi channel conflict to pop up dialog */
...
#ifdef CONFIG_MTK_P2P
	wpa_s->global->p2p_group_formation = NULL;
	wpa_s->global->p2p->inv_op_freq = 0;
	p2p_flush(wpa_s->global->p2p);
	if(!os_strcmp(gtype, "client")) {
		os_get_reltime(&p2p->f_go.remove_time);
		wpa_printf(MSG_DEBUG, "P2P: f_go bssid: " MACSTR ", remove_time: %d sec %d usec",
			MAC2STR(p2p->f_go.bssid), p2p->f_go.remove_time.sec, p2p->f_go.remove_time.usec);
	}
...
#ifdef CONFIG_MTK_P2P
void wpas_send_action_done(void *ctx)
#else
...
#endif
{
	...
	if (!wpa_s->global->p2p_disabled && wpa_s->global->p2p)
		p2p_clear_after_sd_tx(wpa_s->global->p2p);
#endif
}
...
#ifdef CONFIG_MTK_P2P
char *wifi_vendor_info_hex(const struct wpabuf *vendor)
{
	char *info = NULL;
	const u8 *data = NULL;
	size_t buflen;
	buflen = wpabuf_len(vendor);
	info = os_zalloc(buflen * 2 + 1);
	if (!info)
		return NULL;
	data = wpabuf_head_u8(vendor);
	wpa_snprintf_hex(info, buflen * 2 + 1, data, buflen);
	return info;
}

char *vendor_info_hex_alloc(struct p2p_data *p2p, const u8 *addr)
{
	struct p2p_device *dev = NULL;
	struct p2p_peer_info *info = NULL;

	dev = p2p_get_device(p2p, addr);
	if (dev)
		info = &dev->info;
	if (info && info->vendor_elems)
		return wifi_vendor_info_hex(info->vendor_elems);

	return NULL;
}

void vendor_info_hex_free(char *m)
{
	os_free(m);
}
#endif
			   ...
			   int new_device)
{
...
#ifdef CONFIG_MTK_P2P
	char *vendor_elems_info_hex = NULL;
#endif
	...
	if (info->vendor_elems)
		vendor_elems_info_hex =
			wifi_vendor_info_hex(info->vendor_elems);
#endif
	...
	wpa_msg_global(wpa_s, MSG_INFO, P2P_EVENT_DEVICE_FOUND MACSTR
			...
			vendor_elems_info_hex ? " vendor_elems_info=0x" : "",
			vendor_elems_info_hex ? vendor_elems_info_hex : "",
		       new_device);
#else
	wpa_msg_global(wpa_s, MSG_INFO, P2P_EVENT_DEVICE_FOUND MACSTR
		       " p2p_dev_addr=" MACSTR
			       ...
			       size_t group_id_len)
{
	...
	int res;
#ifdef CONFIG_MTK_P2P
	struct p2p_data *p2p_vendor = NULL;
	char *vendor_elems_hex = NULL;
	p2p_vendor = wpa_s->global->p2p;
	vendor_elems_hex = vendor_info_hex_alloc(p2p_vendor, peer);
#endif
	if (group_id) {
	...
	res = os_snprintf(params, sizeof(params), " p2p_dev_addr=" MACSTR
		    ...
		    group ? " group=" : "",
		    group ? group->ifname : "",
		    vendor_elems_hex ? " vendor_elems=1 vendor_elems_info=0x" : "",
		    vendor_elems_hex ? vendor_elems_hex : "");
#else
	res = os_snprintf(params, sizeof(params), " p2p_dev_addr=" MACSTR
			  " pri_dev_type=%s name='%s' config_methods=0x%x "
...
#ifdef CONFIG_MTK_P2P
	if (vendor_elems_hex)
		vendor_info_hex_free(vendor_elems_hex);
#endif
}
				     ...
				     int op_freq)
{
	...
	char *vendor_elems_hex = NULL;
	p2p_vendor = wpa_s->global->p2p;
	vendor_elems_hex = vendor_info_hex_alloc(p2p_vendor, sa);
#endif
	...
	if (!s) {
		if (bssid) {
			wpa_msg(wpa_s, MSG_INFO, P2P_EVENT_INVITATION_RECEIVED
				...
				"wfd_extend_capa=0x%x wfd_flags=0x%x%s%s",
				MAC2STR(sa), MAC2STR(go_dev_addr),
				MAC2STR(bssid),
				wfd_params->WfdEnable,
				wfd_params->WfdDevInfo,
				wfd_params->WfdControlPort,
				wfd_params->WfdMaximumTp,
				MAC2STR(wfd_params->WfdAssociatedBssid),
				wfd_params->WfdCoupleSinkStatus,
				MAC2STR(wfd_params->WfdCoupleSinkAddress),
				wfd_params->WfdExtendCap,
				wfd_params->WfdFlag,
				vendor_elems_hex ? " vendor_elems=1 vendor_elems_info=0x" : "",
				vendor_elems_hex ? vendor_elems_hex : "");
		} else {
	...
	wpa_msg(wpa_s, MSG_INFO, P2P_EVENT_INVITATION_RECEIVED "sa=" MACSTR
		...
		if (!s) {
			if (bssid)
				wpa_msg(wpa_s, MSG_INFO, P2P_EVENT_INVITATION_RECEIVED
					"sa=" MACSTR " go_dev_addr=" MACSTR
					" bssid=" MACSTR " unknown-network wfd_en=0%s%s",
					MAC2STR(sa), MAC2STR(go_dev_addr),
					MAC2STR(bssid),
					vendor_elems_hex ? " vendor_elems=1 vendor_elems_info=0x" : "",
					vendor_elems_hex ? vendor_elems_hex : "");
...
#ifdef CONFIG_MTK_P2P
	/*
	* Disable wfd setting cmd in driver
	* as firmware not need these infomation
	* anymore
	*/
#if 0
...
static int wpas_check_freq_conflict(struct wpa_supplicant *wpa_s, int freq)
{
	...
	struct wpa_used_freq_data *freqs;
#ifdef CONFIG_MTK_SCC
	if (wpa_s->global->p2p->p2p_use_mcc) {
		wpa_printf(MSG_DEBUG, "p2p_use_mcc, ignore freq conflict check");
		return 0;
	}
#endif
...
#ifdef CONFIG_MTK_P2P
	/*
	 * ALPS01439965 Flush Bss here,
	 * supplicant will re-scan before connect
	 */
	wpa_bss_flush(wpa_s);
#endif
...
#ifdef CONFIG_MTK_P2P_5G
	if (wpa_s->global->p2p->inv_op_freq > 0) {
		...
		wpa_printf(MSG_DEBUG, "inv_op_freq: %d invalid, use neg_freq",
			wpa_s->global->p2p->inv_op_freq);
		return freq;
	}
#else
...
#ifdef CONFIG_MTK_SCC_MCC
				if (wpa_s->global->p2p->p2p_use_mcc) {
					wpa_printf(MSG_DEBUG, "P2P: Use MCC to build connection, skip operating channel check");
					return 0;
				}
#endif
				return -1;
		       ...
		       int freq, int ht40, int vht)
{
	...
	if (radio_work_pending(wpa_s, "p2p-scan")) {
		wpa_printf(MSG_DEBUG, "P2P: remove p2p scan before setup GO");
		radio_remove_works(wpa_s, "p2p-scan", 0);
	}
#endif
...
static void
wps_p2p_beamplus_go_reserve_timeout(void *eloop_ctx, void *timeout_ctx)
{
...
void
wpas_p2p_beamplus_go_reserve_register_timeout(struct wpa_supplicant *wpa_s)
{
	struct p2p_data *p2p;
...
static void
wps_p2p_beamplus_go_ready_timeout(void *eloop_ctx, void *timeout_ctx)
{
	struct wpa_supplicant *wpa_s = eloop_ctx;
...
void
wpas_p2p_beamplus_go_ready_register_timeout(struct wpa_supplicant *wpa_s)
{
...
#ifdef CONFIG_MTK_P2P
int
wpas_wfd_data_process(struct wpa_supplicant *wpa_s,
		struct wfd_data_s *p_wfd_data_cmd)
{
	...
	wpa_printf(MSG_DEBUG, "WFD: wpas_wfd_data_process %u "
			"wfd_dev_info 0x%x wfd_ctrl_port %u",
			p_wfd_data_cmd->WfdEnable,
			p_wfd_data_cmd->WfdDevInfo,
...
int
wpas_wfd_scan_result_text(const u8 *ies,
			size_t ies_len, char *buf,
			char *end)
{
