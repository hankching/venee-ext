...
#ifdef CONFIG_MTK_P2P
	/*
	 * Here is a temp patch, actually p2p->pending_action_state has a severe issue, if a new action need to do,
	 * it often override this variable, and cause the tx status can't be handled correctly[it will be tackled in Everest]
	 *
	 * Here the flow actually should be:
	 * if (p2p->pending_action_state) {
	 *	do nothing!!!
	 *	Here: should not clear it! (the previous action has not received the tx_status,
	 *	maybe sth important still need to do in tx_status, after all, sd repsonse need to do more after receiving tx_status)
	 * } else {
	 * p2p->pending_action_state = P2P_NO_PENDING_ACTION;
	 * }
	 *
	 * Following this flow, just comment out the following code.
	 *
	 * p2p->pending_action_state = P2P_NO_PENDING_ACTION;
	 */
#endif
