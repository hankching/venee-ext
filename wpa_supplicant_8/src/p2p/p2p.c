...
#ifdef CONFIG_MTK_P2P
static void p2p_sd_timeout(void *eloop_ctx, void *timeout_ctx);
#endif
/*
...
#ifdef CONFIG_MTK_P2P
void p2p_update_peer_vendor_elems(struct p2p_device *dev, const u8 *ies,
					 size_t ies_len)
#else
...
#ifdef CONFIG_MTK_P2P
	do {
		if (is_zero_ether_addr(p2p->f_go.bssid)) {
			p2p_dbg(p2p, "f_go bssid is null");
			break;
		}
		if (os_memcmp(p2p->f_go.bssid, addr, ETH_ALEN)) {
			p2p_dbg(p2p, "dev bssid: " MACSTR " is not same with f_go", MAC2STR(addr));
			break;
		}
		if (!(dev->info.group_capab & 0x01)) {
			p2p_dbg(p2p, "f_go is already not a GO");
			os_memset(&p2p->f_go, 0, sizeof(struct former_go));
			break;
		}
		if (rx_time->sec - p2p->f_go.remove_time.sec <= 5) {
			p2p_dbg(p2p, "f_go is still a GO in 5sec, rx_time.sec: %ld, usec: %ld", rx_time->sec, rx_time->usec);
			os_memset(&p2p->f_go, 0, sizeof(struct former_go));

			return 0;
		} else {
			p2p_dbg(p2p, "f_go is still a GO after 5sec");
			os_memset(&p2p->f_go, 0, sizeof(struct former_go));
		}
	} while(0);
#endif
...
static int p2p_run_after_scan(struct p2p_data *p2p)
{
...
#ifdef CONFIG_MTK_P2P
	if (op == P2P_AFTER_SCAN_NOTHING &&
		p2p->in_beam_scan)
		op = P2P_AFTER_SCAN_SCAN;
#endif
	switch (op) {
	...
	case P2P_AFTER_SCAN_CONNECT:
		...
		return 1;
#ifdef CONFIG_MTK_P2P
	case P2P_AFTER_SCAN_SCAN:
		p2p_dbg(p2p, "BeamPlus: skipping listen, continue scan to "
			"improve efficiency of scan");
		p2p_search(p2p);
		return 1;
#endif
	}
		     ...
		     size_t len, int p2p_group, struct wpabuf *p2p_ie)
{
	...
	if (tmplen > len) {
#ifdef CONFIG_MTK_P2P
		p2p_dbg(p2p, "FATAL: tmplen > len, will cause p2p ie missed in some case!");
#endif
		res = -1;
	} else {
		os_memcpy(buf, wpabuf_head(tmp), tmplen);
...
static void p2p_go_neg_resp_cb(struct p2p_data *p2p, int success)
{
	...
	if (!success && dev && dev->go_neg_resp &&
		dev->go_neg_resp_sent <= P2P_GO_NEG_RSP_MAX_RETRY_COUNT) {
		p2p_dbg(p2p, "GO Negotiation Response retry %d",
				dev->go_neg_resp_sent);

		p2p->pending_action_state = P2P_PENDING_GO_NEG_RESPONSE;
		dev->flags |= P2P_DEV_WAIT_GO_NEG_CONFIRM;

		if (os_memcmp(dev->info.p2p_device_addr,
		...
		if (p2p_send_action(p2p, dev->go_neg_resp_freq,
				    ...
				    wpabuf_len(dev->go_neg_resp), 100) >=
			    0) {
				dev->go_neg_resp_sent++;
				return;
		} else {
			p2p_dbg(p2p, "Failed to re-send Neg Resp frame");
			p2p_set_state(p2p, P2P_CONNECT);
			p2p_set_timeout(p2p, 0, 500000);
		}
	} else {
		p2p_set_state(p2p, P2P_CONNECT);
		p2p_set_timeout(p2p, 1, 500000);
	}
#else
	p2p_set_state(p2p, P2P_CONNECT);
	p2p_set_timeout(p2p, 0, 500000);
...
#ifdef CONFIG_MTK_P2P
	if (p2p->p2p_sd_running) {
		if (p2p->after_sd_tx) {
			p2p_dbg(p2p, "Send pending Action frame at p2p_sd completion");
			p2p->cfg->send_action(p2p->cfg->cb_ctx,
				p2p->after_sd_tx->freq,
				p2p->after_sd_tx->dst,
				p2p->after_sd_tx->src,
				p2p->after_sd_tx->bssid,
				(u8 *) (p2p->after_sd_tx + 1),
				p2p->after_sd_tx->len,
				p2p->after_sd_tx->wait_time);
			os_free(p2p->after_sd_tx);
			p2p->after_sd_tx = NULL;
			p2p->p2p_sd_running = 0;
			p2p->after_scan_tx_in_progress = 0;
			p2p_dbg(p2p, "Send pending Action frame done, not completed yet");
			return;
		}
		p2p->p2p_sd_running = 0;
	}
...
static void p2p_state_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	case P2P_SEARCH:
		...
		if (wpa_s->pending_action_tx) {
			p2p_dbg(p2p, "There is a pending action tx, delay search");
			p2p_set_timeout(p2p, 0, 100000);
			break;
		}
#endif
...
#ifdef CONFIG_MTK_P2P
	if (p2p->p2p_sd_running) {
		p2p_dbg(p2p, "Delay Action frame TX until p2p_sd completes");
		if (p2p->after_sd_tx) {
			p2p_dbg(p2p, "Dropped previous pending Action frame TX");
			os_free(p2p->after_sd_tx);
		}
		p2p->after_sd_tx = os_malloc(sizeof(*p2p->after_sd_tx) +
					       len);
		if (p2p->after_sd_tx == NULL)
			return -1;
		p2p->after_sd_tx->freq = freq;
		os_memcpy(p2p->after_sd_tx->dst, dst, ETH_ALEN);
		os_memcpy(p2p->after_sd_tx->src, src, ETH_ALEN);
		os_memcpy(p2p->after_sd_tx->bssid, bssid, ETH_ALEN);
		p2p->after_sd_tx->len = len;
		p2p->after_sd_tx->wait_time = wait_time;
		os_memcpy(p2p->after_sd_tx + 1, buf, len);
		return 0;
	}
#endif
...
void p2p_sd_set_timeout(struct p2p_data *p2p, unsigned int sec, unsigned int usec) {
	p2p_dbg(p2p, "Set sd timeout: %u.%06u sec", sec, usec);
	eloop_cancel_timeout(p2p_sd_timeout, p2p, NULL);
	eloop_register_timeout(sec, usec, p2p_sd_timeout, p2p, NULL);
}

static void p2p_sd_timeout(void *eloop_ctx, void *timeout_ctx) {
	struct p2p_data *p2p = eloop_ctx;
	p2p_dbg(p2p, "after sd tx timeout");
	p2p_clear_after_sd_tx(p2p);
}

void p2p_clear_after_sd_tx(struct p2p_data *p2p)
{
	p2p->p2p_sd_running = 0;
	if (p2p->after_sd_tx) {
		p2p_dbg(p2p, "clear after sd tx");
		os_free(p2p->after_sd_tx);
		p2p->after_sd_tx = NULL;
	}
}
...
int
wfd_process_request_by_policy(struct p2p_data *p2p,
				struct p2p_message *msg,
				u8 *status,
				unsigned int flags)
{
	...
	u8 wfd_enabled = 0;
	if (!p2p->wfd_params) {
		p2p_dbg(p2p, "WFD: wfd_param is NULL");
		return 0;
	}
	p2p_dbg(p2p, "WFD: wfd_process_request_by_policy wfd_en %u",
			p2p->wfd_params->WfdEnable);
	wfd_enabled = p2p->wfd_params->WfdEnable ? 1 : 0;
	if (!wfd_enabled) {
		p2p_dbg(p2p, "WFD: wfd not enabled");
		return 0;
	}
	if (p2p->wfd_params->WfdPolicy & (1<<6)){
		res_status = P2P_SC_FAIL_REJECTED_BY_USER;
		p2p_dbg(p2p, "WFD: device info attribute is worng "
			"Reject all");
		goto fail;
	}
...
int
wfd_process_request_and_switch_role(struct p2p_data *p2p,
				struct p2p_message *msg,
				unsigned int flags)
{
	u8 wfd_enabled = 0;
	...
	p2p_dbg(p2p, "WFD: %s wfd_en %u %p %p", __func__,
		p2p->wfd_params->WfdEnable, msg->wfd_subelems,
		msg->wfd_device_info);
	...
	if (msg->wfd_subelems && msg->wfd_device_info) {
		...
		p2p_dbg(p2p, "WFD: My dev info is 0x%x. Peer is %x",
			p2p->wfd_params->WfdDevInfo, u2DevInfo);

		if ((p2p->wfd_params->WfdDevInfo & 0x3) == 0x3) {
			/* I am dual role */
			if ((u2DevInfo &0x3) == 0x00 /* Peer is Source*/) {
				p2p_dbg(p2p, "WFD: Switch role to primary sink ");
				...
				wpa_drv_driver_cmd(p2p->cfg->cb_ctx,
						"wfd_data_update",
						(char *)(p2p->wfd_params),
						sizeof(struct wfd_data_s));
				/*
				* event to annonce the role
				* is chanaged to P-Sink
				*/
				/*
				* wpa_msg(p2p->cfg->msg_ctx, MSG_INFO,
				* WFD_EVENT_AUTO_SWITCH_ROLE "role=1");
				*/
			} else if ((u2DevInfo &0x3) == 0x01 /* Peer is P-Sink */) {
				p2p_dbg(p2p, "WFD: Switch role to source");
				...
				/*
				* event to annonce the role is
				* chanaged to Source
				*/
				/*
				* wpa_msg(p2p->cfg->msg_ctx, MSG_INFO,
				* WFD_EVENT_AUTO_SWITCH_ROLE "role=0");
				*/
			} else {
				p2p_dbg(p2p, "WFD: Peer role is wrong type(%u) ", (u2DevInfo &0x3));
				p2p_dbg(p2p, "WFD: Switch role to source ");
		...
		if ((p2p->wfd_params->WfdDevInfo & 0x3) == 0x0) {
			...
			p2p_dbg(p2p, "WFD: Final our role is wrong type(%u)",
				(p2p->wfd_params->WfdDevInfo & 0x3));
			p2p_dbg(p2p, "WFD: Switch role to source");
			p2p->wfd_params->WfdDevInfo &= ~0x3;
			p2p->wfd_params_cmd->WfdDevInfo &= ~0x3;
...
int
wfd_scan_result_text(const u8 *ies, size_t ies_len,
			char *buf, char *end)
{
...
void
p2p_copy_wfd_info(struct p2p_device *dev, int probe_req,
			      const struct p2p_message *msg,
			      unsigned int flags)
{
	P_WFD_ATTRI_WFD_DEV_INFO_T prWfdDevInfo;
	...
	if (msg->wfd_associated_bssid) {
		prWfdAssociatedBssid  =
			(P_WFD_ATTRI_ASSOCIATED_BSSID_T) msg->wfd_associated_bssid;
		os_memcpy(dev->info.wfd_data_params.WfdAssociatedBssid,
			prWfdAssociatedBssid->aucAssociatedBssid, ETH_ALEN);
	...
	if (msg->wfd_coupled_sink_info) {
		prWfdCoupledSinkInfo =
			(P_WFD_ATTRI_COUPLED_SINK_STATUS_T) msg->wfd_coupled_sink_info;
		dev->info.wfd_data_params.WfdCoupleSinkStatus =
			prWfdCoupledSinkInfo->ucCoupledSinkStatus;
		os_memcpy(dev->info.wfd_data_params.WfdCoupleSinkAddress,
			prWfdCoupledSinkInfo->aucCoupledSinkAddress, ETH_ALEN);
	...
	if (msg->wfd_ext_capa_bitmap_info) {
		prWfdExtCapaBitmap = (P_WFD_ATTRI_EXT_CAPA_BITMAP_T)
					msg->wfd_ext_capa_bitmap_info;
		dev->info.wfd_data_params.WfdExtendCap = WPA_GET_BE16((u8*)
					(&prWfdExtCapaBitmap->u2WfdExtCapBitmap));
