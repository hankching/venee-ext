...
#ifdef CONFIG_MTK_SCC_MCC
	shared_freq = wpas_get_freq_by_ifname(wpa_s, "wlan0");
	if (shared_freq > 0) {
		...
		if (shared_channel && shared_class) {
			...
			if (p2p_channels_includes(intersection, shared_class,
				shared_channel)) {
			    ...
			    const u8 *data, size_t len, int rx_freq)
{
	...
	if (p2p->channels.reg_classes != 0 && msg.channel_list &&
			...
			p2p_dbg(p2p, "Really has no common channels; "
				"Store peer oper freq from nego req: %d MHz",
				p2p->peer_oper_freq);
			no_common_ch = P2P_SC_FAIL_NO_COMMON_CHANNELS;
	}
	if (p2p->channels.reg_classes == 0) {
		p2p_dbg(p2p, "First p2p GO negotiation, config channels");
	...
	if (dev && dev->flags & P2P_DEV_USER_REJECTED) {
		...
		p2p_dbg(p2p, "Not ready for GO negotiation with " MACSTR,
			MAC2STR(sa));
/*
* [ALPS00397927] Wifi-direct test:
* Group joint test fail randomly by CTS verifier tool
*
* [WLAN][WPA_SUPPLICANT]
* 1. Fix CTS test problem about P2P connection.
*    a. Set listen channel from GO negotiation Request. This patch can
*	fix wrong channel for unkown peer invitation.
*    b. Reject auto-connection if GROUP-REMOVE command comes from user's request
*    c. Set operating channel from GO negotiation Confirmation to prevent from
*	wrong channel operation
...
* [WLAN][WPA_SUPPLICANT]
* 1. Fix CTS test problem about P2P connection.
*    a. Set listen channel from GO negotiation Request. This patch can fix
*	wrong channel for unkown peer invitation.
*    b. Reject auto-connection if GROUP-REMOVE command comes from user's request
*    c. Set operating channel from GO negotiation Confirmation to prevent from
*	wrong channel operation
* 2. Fix CTS test problem about GROUP join.
			     ...
			     const u8 *data, size_t len, int rx_freq)
{
	...
	if (*msg.status) {
		...
		} else if (*msg.status == P2P_SC_FAIL_NO_COMMON_CHANNELS) {
			p2p_dbg(p2p, "No Common channel, need trigger another connection");
			wpa_msg_global(wpa_s, MSG_INFO,
					P2P_EVENT_GO_NEG_FAILURE "status=7 peer_oper_freq=-1");
#endif
		} else {
