				...
				const u8 *data, size_t len, int rx_freq)
{
	...
	if (!go) {
		p2p_dbg(p2p, "Invitation: show respect for GO op channel");
		op_freq = 0;
	}
#endif
				 ...
				 const u8 *data, size_t len)
{
	...
	if (msg.operating_channel) {
		int freq = -1;
		freq = p2p_channel_to_freq(msg.operating_channel[3],
					msg.operating_channel[4]);
		if (freq < 0) {
			p2p_dbg(p2p, "peer used wrong op_class & channel: %d : %d",
				msg.operating_channel[3], msg.operating_channel[4]);
			freq = mtk_channel_to_freq(msg.operating_channel[4]);
		}
		p2p->peer_oper_freq = freq;
	...
	if (msg.operating_channel
		...
		dev->oper_freq = p2p->inv_op_freq = freq;
