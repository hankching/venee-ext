        ...
        case PARSER_DATA_LOCATE:
        {
            ...
            for (i = 0; i < pSMFData->numStreams; i++)
            {
                ...
                if (EAS_SUCCESS != result) {
                    ALOGV("SMF_SetData:  EAS_HWFileSeek stream[%ld] track[%ld] return %ld", i, trackNum, result);
                }
                ...
                if (EAS_SUCCESS != result) {
                    ALOGV("SMF_SetData:  PARSER_DATA_LOCATE, track[%ld] call SMF_HandleSeekTableEvent return %ld", trackNum, result);
                }
...
static EAS_RESULT SMF_HandleSeekTableEvent (S_EAS_DATA *pEASData, S_SMF_DATA *pSMFData, EAS_INT trackNum, EAS_INT parserMode, EAS_U32 position)
{
    ...
    if (!eventList || !eventCount) {
        ALOGW("SMF_HandleSeekTableEvent[%p] eventList: %p eventCount: %lu", pSMFData, eventList, eventCount);
        return EAS_ERROR_INVALID_PARAMETER;
    }
...
void SMF_DumpStreamInfo(S_EAS_DATA *pEASData, EAS_VOID_PTR pInstData, EAS_INT parserMode)
{
    ...
    for (i = 0; i < pSMFData->numStreams; i++)
    {
        ALOGV("SMF_DumpStreamInfo: pSMFData->streams[%d].ticks = %lu",i,pSMFData->streams[i].ticks);
