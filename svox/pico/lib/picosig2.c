...
//ALPS01771022 mtk01981 modified @20141126    
//In the original code, it clears the XXr buffer from byte 4*m1 to byte 4*(PICODSP_FFTSIZE + 1).
//I modify as clearing the XXr buffer from byte 4*m1 to byte 4*(PICODSP_FFTSIZE). 
//Because in the "pico_status_t sigAllocate(picoos_MemoryManager mm, sig_innerobj_t *sig_inObj)"
//the allocation size is only sizeof(picoos_int32) * PICODSP_FFTSIZE
